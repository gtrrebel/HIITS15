...
class Polygamma1(UnaryScalarOp):
    """
    Polygamma_1 function
    """
    @staticmethod
    def st_impl(x):
        if isinstance(x, float):
            return scipy.special.polygamma(1, x).item(0)
        return scipy.special.polygamma(1,x)

    def impl(self, x):
        if imported_scipy_special:
            return Polygamma1.st_impl(x)
        else:
            super(Polygamma1, self).impl(x)
    ...
    def __hash__(self):
        return hash(type(self))
polygamma1 = Polygamma1(upgrade_to_float, name='polygamma1')
